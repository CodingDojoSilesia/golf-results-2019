...
## creates egg from "instrucions" list
def egg():
    r,d='',[(22,30),(19,22),(17,19),(15,17),(13,15),12,11,10,9,8,7,6,5,4,4,3,2,2,2,1,1]+[0]*13+[1,2,2,(3,5),5,(6,8),(8,10),(10,12),(12, 15),(15, 19),(19,30)]
    l=[eval(f'rep("{" "*30}","@",{v[0]},{v[1]})') if type(v) is tuple else eval(f'rep("{" "*30}","@",{v},{v+1})') for v in d] ## creates first half of every line
    return [x+x[::-1] for x in l] ## merge first and second half of every line
...
## draw all patterns in one string
def f_p(args):
    p=[]
    for a in args:
        f,*r=a.split(",") ## get function name and arguments
        r=[int(x) if x.isnumeric() else x for x in r] ## prepare arguments for function
        p+=globals()[f](*r)
    return p

## draw egg and pattern on it
def p_e(args):
    eg,r,p=egg(),'',f_p(args)
    for n, l in enumerate(eg):
        if n!=0 and n!=44: ## skip loop for first and last line
            b=l.index("@")+l.count("@")//2
...
if __name__=="__main__":
    ## if there's no arguments draw empty egg
    try:a=sys.argv[1]
    except IndexError:
        r=''
        for x in egg():r+=x+'\n'
